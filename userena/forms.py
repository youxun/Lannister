...
from userena import boostrap as userena_boostrap
import random


attrs_dict={'class':'required'}
attrs_dict_text = {'class': 'required'}
attrs_dict_password={'class': 'required'}
attrs_dict_checkbox={'class':'required'}

if userena_settings.USERENA_BOOSTRAP:
    attrs_dict=userena_boostrap.BOOSTRAP_DEFAULT
    attrs_dict_text = userena_boostrap.BOOSTRAP_TEXTINPUT
    attrs_dict_password = userena_boostrap.BOOSTRAP_PASSWORD
    attrs_dict_checkbox = userena_boostrap.BOOSTRAP_CHECKBOX
...
class SignupForm(forms.Form):
    ...
    username = forms.RegexField(regex=USERNAME_RE,
                                max_length=30,
                                widget=forms.TextInput(attrs=attrs_dict_text),
                                label=_("Username"),
                                error_messages={'invalid': _('Username must contain only letters, numbers, dots and underscores.')})
    email = forms.EmailField(widget=forms.TextInput(attrs=dict(attrs_dict_text,
                                                               maxlength=75)),
                             label=_("Email"))
    password1 = forms.CharField(widget=forms.PasswordInput(attrs=attrs_dict_password,
                                                           render_value=False),
                                label=_("Create password"))
    password2 = forms.CharField(widget=forms.PasswordInput(attrs=attrs_dict_password,
                                                           render_value=False),
...
class SignupFormTos(SignupForm):
    ...
    tos = forms.BooleanField(widget=forms.CheckboxInput(attrs=attrs_dict_checkbox),
...
class AuthenticationForm(forms.Form):
    ...
    password = forms.CharField(label=_("Password"),
                               widget=forms.PasswordInput(attrs=attrs_dict_password, render_value=False))
